...
control vuelve solo — la arista de salida la pone `vendedores/grafo.py`.
...
El resto del estado —`perfil`, `conversacion`— sí viaja, y de ahí sale el
`vendedor_id` que el modelo no puede pasar como argumento.

De vuelta salen dos cosas:

    un ToolMessage    con el texto final de este subgrafo, contra el
                      tool_call_id de la delegación
    media_pendiente   las fotos, que tiene reducer de suma y las manda el
                      worker al final del turno
...
Lo hace `_envolver_area` en vendedores/grafo.py.

── Por qué un modelo chico ────────────────────────────────────────────────────

Este nodo no razona sobre el negocio: elige entre cuatro tools y devuelve lo
que salió. El razonamiento —cruzar áreas, decidir qué se le dice al asesor— es
del orquestador, que sí corre con un modelo grande.
"""
import os
...
from vendedores.agentes.productos.prompt import SYSTEM
